    ...
    quote! {
        ...
        pub mod #server_mod {
            ...
            impl<T, B> tonic::codegen::Service<http::Request<B>> for #server_service<T>
                where
                    T: #server_trait,
                    B: Body + std::marker:: + 'static,
                    B::Error: Into<StdError> + std::marker:: + 'static,
            {
    ...
    for method in service.methods() {
        ...
        let method = match (
            ...
            (false, true, false) => {
                ...
                quote! {
                    #stream_doc
                    type #stream: tonic::codegen::tokio_stream::Stream<Item = std::result::Result<#res_message, tonic::Status>> + std::marker:: + 'static;
