...
pub fn parse_grpc_timeout(s: &str) -> Result<Duration, GrpcTimeoutError<'_>> {
    ...
    match unit_char {
        'H' => Ok(Duration::from_secs(
            value.checked_mul(3600).ok_or(GrpcTimeoutError::Overflow)?,
        )),
        'M' => Ok(Duration::from_secs(
            value.checked_mul(60).ok_or(GrpcTimeoutError::Overflow)?,
        )),
        'S' => Ok(Duration::from_secs(value)),
        'm' => Ok(Duration::from_millis(value)),
        'u' => Ok(Duration::from_micros(value)),
        'n' => Ok(Duration::from_nanos(value)),
        _ => Err(GrpcTimeoutError::InvalidUnit(unit_char)),
    }
}
...
mod tests {
    ...
    fn test_empty_header() {
        assert_eq!(parse_grpc_timeout(""), Err(GrpcTimeoutError::Empty));
    }
    ...
    fn test_header_without_digits() {
        ...
        assert_eq!(
            ...
            Err(GrpcTimeoutError::NonDigitValue)
        );
    }
    ...
    fn test_invalid_unit() {
        assert_eq!(
            ...
            Err(GrpcTimeoutError::InvalidUnit('f'))
        );
    }
    ...
    fn test_overflow_hours() {
        ...
        assert_eq!(parse_grpc_timeout(&big), Err(GrpcTimeoutError::Overflow));
    }
