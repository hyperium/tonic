...
pub trait Body: sealed::Sealed + Sync + Send {
    /// The body data type.
    type Data: Buf;
...
pub struct BoxBody {
    inner: Pin<Box<dyn Body<Data = BytesBuf, Error = Status> + Sync + Send + 'static>>,
}
...
impl BoxBody {
    ...
    where
        B: Body<Data = BytesBuf, Error = Status> + Sync + Send + 'static,
    {
        BoxBody {
            inner: Box::pin(inner),
        }
