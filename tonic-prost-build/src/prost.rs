...
use quote::ToTokens;
use tonic_build::{code_gen::CodeGenBuilder, compile_settings::CompileSettings, Attributes};
...
impl tonic_build::Method for TonicBuildMethod {
    ...
    fn request_response_name(
                ...
                .any(|ty| self.prost_method.input_type.ends_with(ty))
        {
            ...
            syn::parse_str::<syn::Path>(&format!("{}::{}", proto_path, self.prost_method.input_type))
                .unwrap()
                .to_token_stream()
        };

        let response = if (is_google_type(&self.prost_method.output_type) && !compile_well_known_types)
            || NON_PATH_TYPE_ALLOWLIST
                .iter()
                .any(|ty| self.prost_method.output_type.ends_with(ty))
        {
            self.prost_method.output_type.parse::<TokenStream>().unwrap()
        } else {
            syn::parse_str::<syn::Path>(&format!(
                "{}::{}",
                proto_path, self.prost_method.output_type
            ))
            .unwrap()
            .to_token_stream()
        };
...
pub struct ServiceGenerator {
    ...
    out_dir: PathBuf,
    /// Temporary directory for processing .proto files.
    /// We keep a reference to it to ensure it remains alive until the generator finishes.
    #[allow(dead_code)]
...
impl prost_build::ServiceGenerator for ServiceGenerator {
    fn generate(&mut self, service: Service, buf: &mut String) {
        let tonic_service = TonicBuildService::new(service, self.compile_settings.clone());
...
impl Builder {
    ...
    pub fn message_attribute<P: AsRef<str>, A: AsRef<str>>(mut self, path: P, attribute: A) -> Self {
    ...
    pub fn codec_path(mut self, path: impl AsRef<str>) -> Self {
        self.compile_settings.codec_path = path.as_ref().to_string();
        self
    }

    /// Set the configuration for encoding and decoding.
    pub fn compile_settings(mut self, settings: CompileSettings) -> Self {
        self.compile_settings = settings;
        self
    }
